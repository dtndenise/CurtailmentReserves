...
# Kt and desvios padroes (beta) - Caso 4
eol_kappa_NE_up = CSV.read(scriptPath * "/renovaveis/eol_curva90_NE_UPstat_kt.csv", DataFrame)
eol_kappa_NE_dn = CSV.read(scriptPath * "/renovaveis/eol_curva90_NE_DNstat_kt.csv", DataFrame)
eol_kappa_S_up = CSV.read(scriptPath * "/renovaveis/eol_curva90_S_UPstat_kt.csv", DataFrame)
eol_kappa_S_dn = CSV.read(scriptPath * "/renovaveis/eol_curva90_S_DNstat_kt.csv", DataFrame)

beta_eolUP_NEperc = CSV.read(scriptPath * "/renovaveis/beta_eolUP_NEperc_30b.csv", DataFrame)
beta_eolDN_NEperc = CSV.read(scriptPath * "/renovaveis/beta_eolDN_NEperc_30b.csv", DataFrame)
beta_eolUP_Sperc = CSV.read(scriptPath * "/renovaveis/beta_eolUP_Sperc_30b.csv", DataFrame)
beta_eolDN_Sperc = CSV.read(scriptPath * "/renovaveis/beta_eolDN_Sperc_30b.csv", DataFrame)

eol_kappa_NE_up = Matrix(eol_kappa_NE_up)
eol_kappa_NE_dn = Matrix(eol_kappa_NE_dn)
eol_kappa_S_up  = Matrix(eol_kappa_S_up)
eol_kappa_S_dn  = Matrix(eol_kappa_S_dn)

beta_eolUP_NEperc = Matrix(beta_eolUP_NEperc)
beta_eolDN_NEperc = Matrix(beta_eolDN_NEperc)
beta_eolUP_Sperc = Matrix(beta_eolUP_Sperc)
beta_eolDN_Sperc = Matrix(beta_eolDN_Sperc)
#------------------------------------------------------------------------------
...
# Passagem para semi-horário
eol_kappa_NEup_pat = zeros(pat)
eol_kappa_NEdn_pat = zeros(pat)
eol_kappa_Sup_pat  = zeros(pat)
eol_kappa_Sdn_pat  = zeros(pat)


for h in 1:Int(pat / 2)
    eol_kappa_NEup_pat[2*h-1] = eol_kappa_NE_up[h]
    eol_kappa_NEup_pat[2*h]   = eol_kappa_NEup_pat[2*h-1]
    eol_kappa_NEdn_pat[2*h-1] = eol_kappa_NE_dn[h]
    eol_kappa_NEdn_pat[2*h]   = eol_kappa_NEdn_pat[2*h-1]

    eol_kappa_Sup_pat[2*h-1] = eol_kappa_S_up[h]
    eol_kappa_Sup_pat[2*h]   = eol_kappa_Sup_pat[2*h-1]
    eol_kappa_Sdn_pat[2*h-1] = eol_kappa_S_dn[h]
    eol_kappa_Sdn_pat[2*h]   = eol_kappa_Sdn_pat[2*h-1]
end

# Caso 4
beta_eolUP_perc = zeros(n_eol,Int(pat/2))

beta_eolUP_perc[1,:] = beta_eolUP_NEperc[1,:]
beta_eolUP_perc[2,:] = beta_eolUP_NEperc[2,:]
beta_eolUP_perc[3,:] = beta_eolUP_NEperc[3,:]
beta_eolUP_perc[4,:] = beta_eolUP_NEperc[4,:]
beta_eolUP_perc[5,:] = beta_eolUP_NEperc[5,:]
beta_eolUP_perc[6,:] = beta_eolUP_Sperc[1,:]
beta_eolUP_perc[7,:] = beta_eolUP_Sperc[2,:]
beta_eolUP_perc[8,:] = beta_eolUP_Sperc[3,:]
beta_eolUP_perc[9,:] = beta_eolUP_NEperc[6,:]

beta_eolDN_perc = zeros(n_eol,Int(pat/2))

beta_eolDN_perc[1,:] = beta_eolDN_NEperc[1,:]
beta_eolDN_perc[2,:] = beta_eolDN_NEperc[2,:]
beta_eolDN_perc[3,:] = beta_eolDN_NEperc[3,:]
beta_eolDN_perc[4,:] = beta_eolDN_NEperc[4,:]
beta_eolDN_perc[5,:] = beta_eolDN_NEperc[5,:]
beta_eolDN_perc[6,:] = beta_eolDN_Sperc[1,:]
beta_eolDN_perc[7,:] = beta_eolDN_Sperc[2,:]
beta_eolDN_perc[8,:] = beta_eolDN_Sperc[3,:]
beta_eolDN_perc[9,:] = beta_eolDN_NEperc[6,:]
...
for w = 1:n_eol
    for t = 1:pat
        if oper_eol[w, 8] == "S"
            kappa_eol_ResUP[w, t] = beta_eolUP_perc_pat[w,t] * eol_kappa_Sup_pat[t]
            kappa_eol_ResDN[w, t] = beta_eolDN_perc_pat[w,t] * eol_kappa_Sdn_pat[t]
        elseif oper_eol[w, 8] == "NE"
            kappa_eol_ResUP[w, t] = beta_eolUP_perc_pat[w,t] * eol_kappa_NEup_pat[t]
            kappa_eol_ResDN[w, t] = beta_eolDN_perc_pat[w,t] * eol_kappa_NEdn_pat[t]
        end
    end
end

# Verificação de kappa
sum(kappa_eol_ResUP[6:8,:],dims=1)
eol_kappa_Sup_pat'

sum(kappa_eol_ResUP[1:5,:],dims=1).+kappa_eol_ResUP[9,:]'
eol_kappa_NEup_pat'

sum(kappa_eol_ResDN[6:8,:],dims=1)
eol_kappa_Sdn_pat'

sum(kappa_eol_ResDN[1:5,:],dims=1).+kappa_eol_ResDN[9,:]'
eol_kappa_NEdn_pat'
...
#-------------------------------------------------------------------------------
    ...
    # Reservas hidráulicas endógenas (Caso 5)
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados 30b - Caso 5 (kit correlacionado - beta_it) =========")
...
#########################################
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_PcutEOL.csv", PcutEOL, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dliq.csv", d, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dbrut.csv", dbrut, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_pload_brut.csv", pload_rbrut, ',')  
# pload_rliqfinal = pload_rbrut - pload_rliq
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolSup.csv", Psup_eol, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolInf.csv", Pinf_eol, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UTEGer.csv", PI, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UHEGer.csv", GI, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCT-ReserveUP.csv", RUP, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCT_ReserveDN.csv", RDN, ',')
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCH_ReserveUP.csv", RUPH, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCH_ReserveDN.csv", RDNH, ',')
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_XUHE.csv", X, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_VolUHE.csv", VOL, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_Z.csv", Z, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_ALPHA.csv", ALPHA, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_SENS.csv", β_b, ',') 

#-------------------------------------------------------------------
# Leitura dos resultados - Caso 4

# PI       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UTEGer.csv",DataFrame,header=false)
# GI       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UHEGer.csv",DataFrame,header=false)
# RUP      = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCT-ReserveUP.csv",DataFrame,header=false)
# RUPH     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCH_ReserveUP.csv",DataFrame,header=false)
# RDNH     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UCH_ReserveDN.csv",DataFrame,header=false)
# X        = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_XUHE.csv",DataFrame,header=false)
# VOL       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_VolUHE.csv",DataFrame,header=false)
# PcutEOL  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_PcutEOL.csv",DataFrame,header=false)
# Psup_eol  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolSup.csv",DataFrame,header=false)
# Pinf_eol  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolInf.csv",DataFrame,header=false)
# d         = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dliq.csv",DataFrame,header=false)
# dbrut     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dbrut.csv",DataFrame,header=false)
# pload_rbrut = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_pload_brut.csv",DataFrame,header=false)
# Z      = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_Z.csv",DataFrame,header=false)
# ALPHA     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_ALPHA.csv",DataFrame,header=false)
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho 30barras - Caso 5 Verificado ==============")
...
# Exportação dos resultados 
#########################################
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_PcutEOL2_Ver.csv", PcutEOL2, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolSup_Ver.csv", Ver_eol, ',') 

# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dbrut_Ver.csv", dbrut_ver, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_CorteCarga_Ver.csv", Corte_carga, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_CorteCargaBus_Ver.csv", Corte_carga_bus, ',') 

# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UHEGer_Ver.csv", Ger_hidrTR, ',')
# writedlm(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_Z2_Ver.csv", Z2, ',')

# #-------------------------------------------------------------------
# # Leitura dos resultados - Caso 4

# PcutEOL2        = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_PcutEOL2_Ver.csv",DataFrame,header=false)
# Ver_eol         = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_GerEolSup_Ver.csv",DataFrame,header=false)
# dbrut_ver       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_dbrut_Ver.csv",DataFrame,header=false)
# Corte_carga     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_CorteCarga_Ver.csv",DataFrame,header=false)
# Corte_carga_bus = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_CorteCargaBus_Ver.csv",DataFrame,header=false)
# Ger_hidrTRZ     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_UHEGer_Ver.csv",DataFrame,header=false)
# Z2              = CSV.read(scriptPath * "/Resultados/RPOzero/Caso4/Result30b_Caso4_Z2_Ver.csv",DataFrame,header=false)
