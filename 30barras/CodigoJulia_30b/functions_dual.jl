...
DUAL_UCPDO = Model(Gurobi.Optimizer)  ####
...
if nlinhas >=1
    ...
    #-------------------------------------------------------------------------------
        ...
        rest_add1[l=1:nlinhas,t=2:int], sum(restadd[l,j+3]*(sum(pit[i,t] for i=1:n_ugen if pi_bus[i] ==
                                ...
                                nobus[j])- pload_r[j,t]) for j=1:nbus if restadd[l,j+3] >= 0.1 || restadd[l,j+3] <= -0.1) <= restadd[l,3]
        rest_add2[l=1:nlinhas,t=2:int], -restadd[l,3] <= sum(restadd[l,j+3]*(sum(pit[i,t] for i=1:n_ugen if pi_bus[i] ==
                                nobus[j]) + sum(gi[h,t] for h=1:n_ugh if gi_bus[h] ==
                                nobus[j])- pload_r[j,t]) for j=1:nbus if restadd[l,j+3] >= 0.1 || restadd[l,j+3] <= -0.1)
    end)
end
status = optimize!(DUAL_UCPDO)  ####
PITT = value.(pit) ####
GIT = value.(gi) ####
#FOL = getvalue(folga)
#THETA = getvalue(θ)
...
LFLUX2 = 0
RESTUP = dual.(r_sub) ####
RESHUP = dual.(r_sub_ugh) ####
if nlinhas >=1
    REST1 = dual.(rest_add2) ####
    REST2 = dual.(rest_add2) ####
else
    REST1 = 0
    REST2 = 0
end
