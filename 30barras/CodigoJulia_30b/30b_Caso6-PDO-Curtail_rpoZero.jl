...
println("====================================================================================================")
println("         Model Day-ahead Dispatch - Case 6        ")
println("====================================================================================================")
#-------------------------------------------------------------------------------
...
# Variação sem correlação (kit) - Caso 5
deltaG_UPeol1 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol1_30b.csv", DataFrame)
deltaG_UPeol2 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol2_30b.csv", DataFrame)
deltaG_UPeol3 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol3_30b.csv", DataFrame)
deltaG_UPeol4 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol4_30b.csv", DataFrame)
deltaG_UPeol5 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol5_30b.csv", DataFrame)
deltaG_UPeol6 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol6_30b.csv", DataFrame)
deltaG_UPeol7 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol7_30b.csv", DataFrame)
deltaG_UPeol8 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol8_30b.csv", DataFrame)
deltaG_UPeol9 = CSV.read(scriptPath * "/renovaveis/deltaG_UPeol9_30b.csv", DataFrame)

deltaG_DNeol1 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol1_30b.csv", DataFrame)
deltaG_DNeol2 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol2_30b.csv", DataFrame)
deltaG_DNeol3 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol3_30b.csv", DataFrame)
deltaG_DNeol4 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol4_30b.csv", DataFrame)
deltaG_DNeol5 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol5_30b.csv", DataFrame)
deltaG_DNeol6 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol6_30b.csv", DataFrame)
deltaG_DNeol7 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol7_30b.csv", DataFrame)
deltaG_DNeol8 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol8_30b.csv", DataFrame)
deltaG_DNeol9 = CSV.read(scriptPath * "/renovaveis/deltaG_DNeol9_30b.csv", DataFrame)
...
deltaG_UPeolh = zeros(n_eol,Int(pat/2))
deltaG_UPeolh[1,:] = deltaG_UPeol1
deltaG_UPeolh[2,:] = deltaG_UPeol2
deltaG_UPeolh[3,:] = deltaG_UPeol3
deltaG_UPeolh[4,:] = deltaG_UPeol4
deltaG_UPeolh[5,:] = deltaG_UPeol5
deltaG_UPeolh[6,:] = deltaG_UPeol6
deltaG_UPeolh[7,:] = deltaG_UPeol7
deltaG_UPeolh[8,:] = deltaG_UPeol8
deltaG_UPeolh[9,:] = deltaG_UPeol9

deltaG_DNeolh = zeros(n_eol,Int(pat/2))
deltaG_DNeolh[1,:] = deltaG_DNeol1
deltaG_DNeolh[2,:] = deltaG_DNeol2
deltaG_DNeolh[3,:] = deltaG_DNeol3
deltaG_DNeolh[4,:] = deltaG_DNeol4
deltaG_DNeolh[5,:] = deltaG_DNeol5
deltaG_DNeolh[6,:] = deltaG_DNeol6
deltaG_DNeolh[7,:] = deltaG_DNeol7
deltaG_DNeolh[8,:] = deltaG_DNeol8
deltaG_DNeolh[9,:] = deltaG_DNeol9

# Passagem para semi-horário
Psup_eol = zeros(n_eol,pat)
deltaG_UPeol = zeros(n_eol,pat)
deltaG_DNeol = zeros(n_eol,pat)

for w in 1:n_eol
    for h in 1:Int(pat/2)
        Psup_eol[w,2*h-1]     = Psup_eolh[w,h]
        Psup_eol[w,2*h]       = Psup_eol[w,2*h-1]
        deltaG_UPeol[w,2*h-1] = deltaG_UPeolh[w,h]
        deltaG_UPeol[w,2*h]   = deltaG_UPeol[w,2*h-1]
        deltaG_DNeol[w,2*h-1] = deltaG_DNeolh[w,h]
        deltaG_DNeol[w,2*h]   = deltaG_DNeol[w,2*h-1]
    end 
end
Psup_eol = max.(Psup_eol, 0) 

kappa_eol_ResUP = -deltaG_UPeol
kappa_eol_ResDN = deltaG_DNeol

#-------------------------------------------------------------------------------
# Criterio de Segurança
#-------------------------------------------------------------------------------
Κg = geral_ucpdo[1,1]
Κl = geral_ucpdo[1,2]
RI=0
...
imp = 0 # Imprimi restrições imp = 1 , não imprimi imp = 
...
#-------------------------------------------------------------------------------
    ...
    # Reservas hidráulicas endógenas (Caso 6)
...
while p_viol >= 0.01 && contador <=cont_max  # Se a violação é acima de 1%, adiciona uma nova restrição.
    ...
    if p_viol >= 0.01
        ...
        for t = 2:int
            rest_rede1 = @constraint(UCPDO, sum(β_b[lmax,j]*(sum(pit[i,t] for i=1:n_ugen if pi_bus[i] == nobus[j]) + #β_b1[lmax,j]*
                                                            sum(gi[h,t] for h=1:n_ugh if gi_bus[h] == nobus[j]) +
                                                            sum(Psup_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) - 
                                                            sum(pcut_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) -
                                                            pload_rbrut[j,t]) for j=1:nbus)  <= capalin_rc[lmax]) # if β_b1[lmax,j] >= 0.1 || β_b1[lmax,j] <= -0.1
                                                                    
            rest_rede2 = @constraint(UCPDO, -capalin_rc[lmax] <= sum(β_b[lmax,j]*(sum(pit[i,t] for i=1:n_ugen if pi_bus[i] == nobus[j]) + #β_b1[lmax,j]*
                                                                                sum(gi[h,t] for h=1:n_ugh if gi_bus[h] == nobus[j]) +
                                                                                sum(Psup_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) -
                                                                                sum(pcut_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) - 
                                                                                pload_rbrut[j,t]) for j=1:nbus )) # if β_b1[lmax,j] >= 0.1 || β_b1[lmax,j] <= -0.1
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados 30b - Caso 6 (kit não correlacionado) =========")
...
#########################################
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_PcutEOL.csv", PcutEOL, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_dliq.csv", d, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_dbrut.csv", dbrut, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_pload_brut.csv", pload_rbrut, ',')  
# pload_rliqfinal = pload_rbrut - pload_rliq
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_GerEolSup.csv", Psup_eol, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_GerEolInf.csv", Pinf_eol, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UTEGer.csv", PI, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UHEGer.csv", GI, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCT-ReserveUP.csv", RUP, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCH_ReserveUP.csv", RUPH, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCH_ReserveDN.csv", RDNH, ',')
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_XUHE.csv", X, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_VolUHE.csv", VOL, ',')  
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_Z.csv", Z, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_ALPHA.csv", ALPHA, ',') 

#-------------------------------------------------------------------
# Leitura dos resultados - Caso 5

# PI       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UTEGer.csv",DataFrame,header=false)
# GI       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UHEGer.csv",DataFrame,header=false)
# RUP      = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCT-ReserveUP.csv",DataFrame,header=false)
# RUPH     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCH_ReserveUP.csv",DataFrame,header=false)
# RDNH     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UCH_ReserveDN.csv",DataFrame,header=false)
# X        = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_XUHE.csv",DataFrame,header=false)
# VOL       = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_VolUHE.csv",DataFrame,header=false)
# PcutEOL  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_PcutEOL.csv",DataFrame,header=false)
# Psup_eol  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_GerEolSup.csv",DataFrame,header=false)
# Pinf_eol  = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_GerEolInf.csv",DataFrame,header=false)
# d         = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_dliq.csv",DataFrame,header=false)
# dbrut     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_dbrut.csv",DataFrame,header=false)
# pload_rbrut = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_pload_brut.csv",DataFrame,header=false)
# Z      = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_Z.csv",DataFrame,header=false)
# ALPHA     = CSV.read(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_ALPHA.csv",DataFrame,header=false)
...
# Variação da carga ~N com variância 4% positiva e 2.5% negativa
...
while p_viol >= 0.01 && contador <=cont_max  # Se a violação é acima de 1%, adiciona uma nova restrição.
    ...
    if p_viol >= 0.01
        ...
        for t = 2:int
            rest_rede1 = @constraint(UCPDO, sum(β_b[lmax,j]*(sum(results_ute[i,t] for i=1:n_ugen if pi_bus[i] == nobus[j]) + #β_b1[lmax,j]*
                                                            sum(ger_hidr_TR[h,t] for h=1:n_ugh if gi_bus[h] == nobus[j]) +
                                                            sum(Ver_eol_pat[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) - 
                                                            sum(pcut_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) -
                                                            pload_rbrut[j,t] + corte_pload[j,t]) for j=1:nbus)  <= capalin_rc[lmax]) # if β_b1[lmax,j] >= 0.1 || β_b1[lmax,j] <= -0.1
                                                                    
            rest_rede2 = @constraint(UCPDO, -capalin_rc[lmax] <= sum(β_b[lmax,j]*(sum(results_ute[i,t] for i=1:n_ugen if pi_bus[i] == nobus[j]) + #β_b1[lmax,j]*
                                                                                sum(ger_hidr_TR[h,t] for h=1:n_ugh if gi_bus[h] == nobus[j]) +
                                                                                sum(Ver_eol_pat[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) -
                                                                                sum(pcut_eol[w,t-1] for w=1:n_eol if eol_bus[w] == nobus[j]) - 
                                                                                pload_rbrut[j,t] + corte_pload[j,t]) for j=1:nbus )) # if β_b1[lmax,j] >= 0.1 || β_b1[lmax,j] <= -0.1
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho 30barras - Caso 6 ==============")
...
# Exportação dos resultados 
#########################################
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_PcutEOL2_Ver.csv", PcutEOL2, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_GerEolSup_Ver.csv", Ver_eol, ',') 

# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_dbrut_Ver.csv", dbrut_ver, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_CorteCarga_Ver.csv", Corte_carga, ',') 
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_CorteCargaBus_Ver.csv", Corte_carga_bus, ',') 

# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_UHEGer_Ver.csv", Ger_hidrTR, ',')
# writedlm(scriptPath * "/Resultados/RPOzero/Caso5/Result30b_Caso5_Z2_Ver.csv", Z2, ',')
