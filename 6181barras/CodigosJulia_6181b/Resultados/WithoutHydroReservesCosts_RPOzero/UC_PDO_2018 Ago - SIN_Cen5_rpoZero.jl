...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_GerCSV_SIN.csv", PI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCH_GerCSV_SIN.csv", GI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_ResUP_SIN.csv", RUP, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_ResDN_SIN.csv", RDN, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCH_X_SIN.csv", X, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_Vol.csv", VOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_PcutEOL_SIN.csv", PcutEOL, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_PcutUFV_SIN.csv", PcutUFV, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_GerEOLSup_SIN.csv", Psup_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_GerEOLInf_SIN.csv", Pinf_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_CI_nsimu_SIN.csv", CI_nsimu, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-DemandaLiq_SIN.csv", d, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-DemandaBrut_SIN.csv", dbrut, ',') #### demanda bruta
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ploadrcbrut.csv", pload_rbrut, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ploadrcliq.csv", pload_r, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_rdnh.csv", RDNH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ruph.csv", RUPH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_Z.csv", Z, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_FCF.csv", ALPHA, ',')

results_ute = PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
X        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCH_X_SIN.csv",DataFrame,header=false)
PcutEOL  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ploadrcliq.csv",DataFrame,header=false)
RDNH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_rdnh.csv",DataFrame,header=false)
RUPH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_ruph.csv",DataFrame,header=false)
Z      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5-Result_FCF.csv",DataFrame,header=false)
...
plot(reservesUP,label="Upward",title="Caso 5 - Hydro reserves dispatch",linewidth=3)
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_ploadrcliq.csv", pload_r, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_Z2.csv", Z2, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_FCF2.csv", ALPHA, ',')

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso5/Caso5_Ver-Result_FCF2.csv",DataFrame,header=false)
...
# Corte de carga
plot(Corte_carga,linewdth=2,color=:red3,title="Case 5 - Load Shedding",label=false)
xlabel!("Half-Hour")
ylabel!("MW")
