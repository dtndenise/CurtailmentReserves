...
println("====================================================================================================")
println("         Model Day-ahead Dispatch - Case 3     ")
println("====================================================================================================")
#-------------------------------------------------------------------------------
# LeiLtura de dados
#-------------------------------------------------------------------------------
...
# GERAÇÃO EÓLICA - Passagem para semi-horário
Psup_eol = zeros(n_eol, pat)
for w in 1:n_eol
    for h in 1:Int(pat / 2)
        Psup_eol[w, 2*h-1] = Psup_eolh[w, h]
...
for w = 1:n_eol
    for t = 1:pat
        if oper_eol[w, 8] == "S"
            kappa_eol_ResUP[w, t] = eol_kappa_Sup_c90_pat[t]
            kappa_eol_ResDN[w, t] = eol_kappa_Sdn_c90_pat[t]
        elseif oper_eol[w, 8] == "NE"
            kappa_eol_ResUP[w, t] = eol_kappa_NEup_c90_pat[t]
            kappa_eol_ResDN[w, t] = eol_kappa_NEdn_c90_pat[t]
        elseif oper_eol[w, 8] == "N"
            kappa_eol_ResUP[w, t] = eol_kappa_Nup_c90_pat[t]
            kappa_eol_ResDN[w, t] = eol_kappa_Ndn_c90_pat[t]
        end
    end
end
...
for s = 1:n_ufv
    for t = 1:pat
        if oper_ufv[s, 8] == "SE"
            kappa_ufv_ResUP[s, t] = ufv_kappa_SEup_c90_pat[t]
            kappa_ufv_ResDN[s, t] = ufv_kappa_SEdn_c90_pat[t]
        elseif oper_ufv[s, 8] == "NE"
            kappa_ufv_ResUP[s, t] = ufv_kappa_NEup_c90_pat[t]
            kappa_ufv_ResDN[s, t] = ufv_kappa_NEdn_c90_pat[t]
        end
    end
end
...
#-------------------------------------------------------------------------------
    ...
    # Reservas hidráulicas exogenas (Despacho 2)
    ...
    #Reservas hidráulicas endogenas 3 (kt)
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados SIN - Cenário 3 =========")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_GerCSV_SIN.csv", PI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCH_GerCSV_SIN.csv", GI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_ResUP_SIN.csv", RUP, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_ResDN_SIN.csv", RDN, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCH_X_SIN.csv", X, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_Vol.csv", VOL, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_PcutEOL_SIN.csv", PcutEOL, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_PcutUFV_SIN.csv", PcutUFV, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_GerEOLSup_SIN.csv", Psup_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_GerEOLInf_SIN.csv", Pinf_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_CI_nsimu_SIN.csv", CI_nsimu, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-DemandaLiq_SIN.csv", d, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-DemandaBrut_SIN.csv", dbrut, ',') #### demanda bruta
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ploadrcbrut.csv", pload_rbrut, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ploadrcliq.csv", pload_r, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_rdnh.csv", RDNH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ruph.csv", RUPH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_Z.csv", Z, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_FCF.csv", ALPHA, ',')

results_ute = PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
X        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCH_X_SIN.csv",DataFrame,header=false)
PcutEOL  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ploadrcliq.csv",DataFrame,header=false)
RDNH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_rdnh.csv",DataFrame,header=false)
RUPH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_ruph.csv",DataFrame,header=false)
Z      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3-Result_FCF.csv",DataFrame,header=false)
...
plot(reservesUP,label="Upward",title="Caso 3 - Hydro reserves dispatch",linewidth=3)
...
#plot(PcutEOL',label=false,title="Wind power curtailment",linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
...
# --------------------------------   Modelo  Caso 3 - Dados Verificados ----------------------------------
UCPDO_ver = Model(Gurobi.Optimizer)

@variable(UCPDO_ver, ger_hidr_TR[h=1:n_ugh,t=1:int])
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho SIN - Caso 3 Dados Verificados ==============") 
println("Corte de carga: ", round(sum(Corte_carga);digits=2)/2," MWh, ", round(sum(Corte_carga)/sum(d_ver)*100;digits=4), " %")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso3/Caso3_Ver-Result_FCF2.csv",DataFrame,header=false)
...
# Corte de carga
plot(Corte_carga,linewdth=2,color=:red3,title="Case 3 - Load Shedding",label=false)
xlabel!("Half-Hour")
ylabel!("MW")

# Cortes
plot(sum(PcutUFV2,dims=1)',label=false,title="Total solar power curtailment - Case 3",color=:gold2,linewidth=2,ylabel="MW", xlabel="Half-Hour")
plot(sum(PcutEOL2,dims=1)',label=false,title="Total wind power curtailment - Case 3",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
#plot(PcutEOL2,label=false,title="Total wind power curtailment",linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
