...
for w = 1:n_eol
    for t = 1:pat
        if oper_eol[w, 8] == "S"
            kappa_eol_ResUP[w, t] = 0.18
            kappa_eol_ResDN[w, t] = 0.18
        elseif oper_eol[w, 8] == "NE"
            kappa_eol_ResUP[w, t] = 0.07
            kappa_eol_ResDN[w, t] = 0.07
        elseif oper_eol[w, 8] == "N"
            kappa_eol_ResUP[w, t] = 0.29
            kappa_eol_ResDN[w, t] = 0.29
        end
    end
end
...
for s = 1:n_ufv
    for t = 1:pat
        if oper_ufv[s, 8] == "SE"
            kappa_ufv_ResUP[s, t] = 0.23
            kappa_ufv_ResDN[s, t] = 0.23
        elseif oper_ufv[s, 8] == "NE"
            kappa_ufv_ResUP[s, t] = 0.36
            kappa_ufv_ResDN[s, t] = 0.36
        end
    end
end
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados SIN - Cenário 2 (fixed reserves updated, solar and curtail) =========")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_GerCSV_SIN.csv", PI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_GerCSV_SIN.csv", GI, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_ResUP_SIN.csv", RUP, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_ResDN_SIN.csv", RDN, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_ResUP_SIN.csv", RUPH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_ResDN_SIN.csv", RDNH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_X_SIN.csv", X, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_VOL_SIN.csv", VOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_PcutEOL_SIN.csv", PcutEOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_PcutUFV_SIN.csv", PcutUFV, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_GerEOLSup_SIN.csv", Psup_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso4-Result_GerEOLInf_SIN.csv", Pinf_eol, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_CI_nsimu_SIN.csv", CI_nsimu, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_DemandaLiq_SIN.csv", d, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_DemandaBrut_SIN.csv", dbrut, ',') #### demanda bruta
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_ploadrcbrut.csv", pload_rbrut, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_ploadrcliq.csv", pload_r, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_Z.csv", Z, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_FCF.csv", ALPHA, ',') 

results_ute=PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
RUPH     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_ResUP_SIN.csv",DataFrame,header=false)
RDNH     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_ResDN_SIN.csv",DataFrame,header=false)
X        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_X_SIN.csv",DataFrame,header=false)
PcutEOL  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_ploadrcliq.csv",DataFrame,header=false)
Z      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Result_FCF.csv",DataFrame,header=false)
...
#----------------------------------------------------------------------
# Reservas hidráulicas
plot(reservesUP,label="Upward",title="Case 2 - Hydro reserves dispatch",linewidth=3)
...
#----------------------------------------------------------------------
# Cortes
plot(sum(PcutUFV,dims=1)',label=false,title="Case 2 - Total solar power curtailment",color=:gold2,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(PcutEOL,dims=1)',label=false,title="Case 2 - Total wind power curtailment",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

# Previsões
plot(sum(Psup_eol,dims=1)',label=false, title="Case 2 - Wind generation forecast for day-ahead scheduling",color=:green,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(Psup_ufv,dims=1)',label=false, title="Case 2 - Solar generation forecast for day-ahead scheduling",color=:gold2,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
...
plot(PcutEOL',title="Case 2 - Wind curtailment for day-ahead dispatch",legend=:outerbottomright,linewidth=2,label=false)

plot(mean(PcutEOL./Psup_eol*100,dims=1)',title="Case 2 - Ratio of Wind Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:green)
xlabel!("Half-Hour")
ylabel!("%")

plot(mean(PcutUFV./Psup_ufv,dims=1)',title="Case 2 - Ratio of Solar Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:gold2)
xlabel!("Half-Hour")
ylabel!("%")
...
heatmap1 = heatmap(perc_eol_cplx, title="Case 2 - Complex WPP \n uncertainty coefficients",xlabel="Half-hour",yflip=true)
plot!(size=[500,700])
ylabel!("WPP")
...
heatmap2 = heatmap(PcutEOL_cplx./GerMax_eol_cplx, title="Case 2 - Ratio of Wind Curtail \n to Installed Capacity",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap2b = heatmap(norm_PcutEOL_GerMaxEOL, title="Case 2 - Ratio of Wind Curtail \n to Installed Capacity Norm",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap2c = heatmap(PcutEOL_cplx./Psup_eol_cplx, title="Case 2 - Ratio of Wind Curtail \n to Forecasted Generation",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap3 = heatmap(PcutEOL_cplx, title="Case 2 - Wind power\n plants' curtailment",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")
...
# Mapas de calor UFV
heatmap(Matrix(perc_ufv_pat1), title="Case 2 - Solar power plants' uncertainty coefficients",xlabel="Half-hour",yflip=true)
plot!(size=[500,700])
ylabel!("WPP")

heatmap(PcutUFV./GerMax_ufv, title="Case 2 - Ratio of Solar Curtail to Installed Capacity",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap(PcutUFV, title="Case 2 - Solar power plants' curtailment",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho SIN - Caso 2 Dados Verificados ==============")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso2/Caso2_Ver-Result_FCF2.csv",DataFrame,header=false)
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 2 - Verified generation")
plot!(size=[900,600])
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 2 - Verified generation - Hydro reserves")

# Corte de carga
plot(Corte_carga,linewdth=2,color=:red3,title="Case 2 - Load Shedding",label=false)
xlabel!("Half-Hour")
ylabel!("MW")
