...
println("====================================================================================================")
println("         Model Day-ahead Dispatch - Case 4     ")
println("====================================================================================================")
#-------------------------------------------------------------------------------
# LeiLtura de dados
#-------------------------------------------------------------------------------
...
# Por submercado - vindo do Cenário 3
eol_kappa_NE_up = CSV.read(scriptPath * "/EOLUFVdata/eol_NE_UP_kt_curva90.csv", DataFrame)
eol_kappa_NE_dn = CSV.read(scriptPath * "/EOLUFVdata/eol_NE_DN_kt_curva90.csv", DataFrame) 

eol_kappa_N_up = CSV.read(scriptPath * "/EOLUFVdata/eol_N_UP_kt_curva90.csv", DataFrame)
eol_kappa_N_dn = CSV.read(scriptPath * "/EOLUFVdata/eol_N_DN_kt_curva90.csv", DataFrame)

eol_kappa_S_up = CSV.read(scriptPath * "/EOLUFVdata/eol_S_UP_kt_curva90.csv", DataFrame)
eol_kappa_S_dn = CSV.read(scriptPath * "/EOLUFVdata/eol_S_DN_kt_curva90.csv", DataFrame)

ufv_kappa_NE_up = CSV.read(scriptPath * "/EOLUFVdata/ufv_NE_UP_kt_curva90.csv", DataFrame)
ufv_kappa_NE_dn = CSV.read(scriptPath * "/EOLUFVdata/ufv_NE_DN_kt_curva90.csv", DataFrame)

ufv_kappa_SE_up = CSV.read(scriptPath * "/EOLUFVdata/ufv_SE_UP_kt_curva90.csv", DataFrame)
ufv_kappa_SE_dn = CSV.read(scriptPath * "/EOLUFVdata/ufv_SE_DN_kt_curva90.csv", DataFrame)
...
# Beta percentual para semi-horario
beta_eolUP_perc_pat = zeros(n_eol, pat)
beta_eolDN_perc_pat = zeros(n_eol, pat)

for w in 1:n_eol
    for h in 1:Int(pat / 2)
        beta_eolUP_perc_pat[w, 2*h-1] = beta_eolUP_perc[w, h]
        beta_eolUP_perc_pat[w, 2*h]   = beta_eolUP_perc_pat[w, 2*h-1]
        beta_eolDN_perc_pat[w, 2*h-1] = beta_eolDN_perc[w, h]
        beta_eolDN_perc_pat[w, 2*h]   = beta_eolDN_perc_pat[w, 2*h-1]
    end
end

beta_ufvUP_perc_pat = zeros(n_ufv, pat)
beta_ufvDN_perc_pat = zeros(n_ufv, pat)

for s in 1:n_ufv
    for h in 1:Int(pat/2)
        beta_ufvUP_perc_pat[s, 2*h-1] = beta_ufvUP_perc[s, h]
        beta_ufvUP_perc_pat[s, 2*h]   = beta_ufvUP_perc_pat[s, 2*h-1]
        beta_ufvDN_perc_pat[s, 2*h-1] = beta_ufvDN_perc[s, h]
        beta_ufvDN_perc_pat[s, 2*h]   = beta_ufvDN_perc_pat[s, 2*h-1]
    end
end


# Duration curve 90%
eol_kappa_NEup_c90 = Matrix{Float32}(eol_kappa_NE_up)
eol_kappa_NEdn_c90 = Matrix{Float32}(eol_kappa_NE_dn)
eol_kappa_Nup_c90  = Matrix{Float32}(eol_kappa_N_up)
eol_kappa_Ndn_c90  = Matrix{Float32}(eol_kappa_N_dn)
eol_kappa_Sup_c90  = Matrix{Float32}(eol_kappa_S_up)
eol_kappa_Sdn_c90  = Matrix{Float32}(eol_kappa_S_dn)

ufv_kappa_NEup_c90 = Matrix{Float32}(ufv_kappa_NE_up)
ufv_kappa_NEdn_c90 = Matrix{Float32}(ufv_kappa_NE_dn)
ufv_kappa_SEup_c90 = Matrix{Float32}(ufv_kappa_SE_up)
ufv_kappa_SEdn_c90 = Matrix{Float32}(ufv_kappa_SE_dn)

eol_kappa_NEup_c90_pat = zeros(pat)
eol_kappa_NEdn_c90_pat = zeros(pat)
eol_kappa_Nup_c90_pat = zeros(pat)
eol_kappa_Ndn_c90_pat = zeros(pat)
eol_kappa_Sup_c90_pat = zeros(pat)
eol_kappa_Sdn_c90_pat = zeros(pat)

ufv_kappa_NEup_c90_pat = zeros(pat)
ufv_kappa_NEdn_c90_pat = zeros(pat)
ufv_kappa_SEup_c90_pat = zeros(pat)
ufv_kappa_SEdn_c90_pat = zeros(pat)

# Duration curve 90% - Passagem para semi-horário
for h in 1:Int(pat / 2)
    eol_kappa_NEup_c90_pat[2*h-1] = eol_kappa_NEup_c90[h]
    eol_kappa_NEup_c90_pat[2*h]   = eol_kappa_NEup_c90_pat[2*h-1]
    eol_kappa_NEdn_c90_pat[2*h-1] = eol_kappa_NEdn_c90[h]
    eol_kappa_NEdn_c90_pat[2*h]   = eol_kappa_NEdn_c90_pat[2*h-1]

    eol_kappa_Nup_c90_pat[2*h-1] = eol_kappa_Nup_c90[h]
    eol_kappa_Nup_c90_pat[2*h]   = eol_kappa_Nup_c90_pat[2*h-1]
    eol_kappa_Ndn_c90_pat[2*h-1] = eol_kappa_Ndn_c90[h]
    eol_kappa_Ndn_c90_pat[2*h]   = eol_kappa_Ndn_c90_pat[2*h-1]

    eol_kappa_Sup_c90_pat[2*h-1] = eol_kappa_Sup_c90[h]
    eol_kappa_Sup_c90_pat[2*h]   = eol_kappa_Sup_c90_pat[2*h-1]
    eol_kappa_Sdn_c90_pat[2*h-1] = eol_kappa_Sdn_c90[h]
    eol_kappa_Sdn_c90_pat[2*h]   = eol_kappa_Sdn_c90_pat[2*h-1]

    ufv_kappa_NEup_c90_pat[2*h-1] = ufv_kappa_NEup_c90[h]
    ufv_kappa_NEup_c90_pat[2*h]   = ufv_kappa_NEup_c90_pat[2*h-1]
    ufv_kappa_NEdn_c90_pat[2*h-1] = ufv_kappa_NEdn_c90[h]
    ufv_kappa_NEdn_c90_pat[2*h]   = ufv_kappa_NEdn_c90_pat[2*h-1]
    
    ufv_kappa_SEup_c90_pat[2*h-1] = ufv_kappa_SEup_c90[h]
    ufv_kappa_SEup_c90_pat[2*h]   = ufv_kappa_SEup_c90_pat[2*h-1]
    ufv_kappa_SEdn_c90_pat[2*h-1] = ufv_kappa_SEdn_c90[h]
    ufv_kappa_SEdn_c90_pat[2*h]   = ufv_kappa_SEdn_c90_pat[2*h-1]
end

#---------------------------------------------------------------------
# kit = beta x kt (Caso 4)
#---------------------------------------------------------------------

kappa_eol_ResUP = zeros(n_eol, pat)
kappa_eol_ResDN = zeros(n_eol, pat)
kappa_ufv_ResUP = zeros(n_ufv, pat)
kappa_ufv_ResDN = zeros(n_ufv, pat)

# beta_eolUP_perc_pat = ones(n_eol,pat)
# beta_eolDN_perc_pat = ones(n_eol,pat)
# beta_ufvUP_perc_pat = ones(n_ufv,pat)
# beta_ufvDN_perc_pat = ones(n_ufv,pat)

for w = 1:n_eol
    for t = 1:pat
        if oper_eol[w, 8] == "S"
            kappa_eol_ResUP[w, t] = beta_eolUP_perc_pat[w,t] * eol_kappa_Sup_c90_pat[t]
            kappa_eol_ResDN[w, t] = beta_eolDN_perc_pat[w,t] * eol_kappa_Sdn_c90_pat[t]
        elseif oper_eol[w, 8] == "NE"
            kappa_eol_ResUP[w, t] = beta_eolUP_perc_pat[w,t] * eol_kappa_NEup_c90_pat[t]
            kappa_eol_ResDN[w, t] = beta_eolDN_perc_pat[w,t] * eol_kappa_NEdn_c90_pat[t]
        elseif oper_eol[w, 8] == "N"
            kappa_eol_ResUP[w, t] = beta_eolUP_perc_pat[w,t] * eol_kappa_Nup_c90_pat[t]
            kappa_eol_ResDN[w, t] = beta_eolDN_perc_pat[w,t] * eol_kappa_Ndn_c90_pat[t]
        end
    end
end
...
for s = 1:n_ufv
    for t = 1:pat
        if oper_ufv[s, 8] == "SE"
            kappa_ufv_ResUP[s, t] = beta_ufvUP_perc_pat[s,t] * ufv_kappa_SEup_c90_pat[t]
            kappa_ufv_ResDN[s, t] = beta_ufvDN_perc_pat[s,t] * ufv_kappa_SEdn_c90_pat[t]
        elseif oper_ufv[s, 8] == "NE"
            kappa_ufv_ResUP[s, t] = beta_ufvUP_perc_pat[s,t] * ufv_kappa_NEup_c90_pat[t]
            kappa_ufv_ResDN[s, t] = beta_ufvDN_perc_pat[s,t] * ufv_kappa_NEdn_c90_pat[t]
        end
    end
end

#---------------------------------------------------------------------
# Previsão - passagem para semi-horario
...
#writedlm(scriptPath * "/Result_PsupUTE_SIN.csv", Psup, ',')
...
# FC eol = 0.63 e FC ufv = 0.34 (ago/2018)
...
#-------------------------------------------------------------------------------
# Requisitos de reservas hidráulicas para controle secundário de frequência
#------------------------------------------------------------------------------
Psup_eolS = zeros(n_eol, pat)
Psup_eolNE = zeros(n_eol, pat)
Psup_eolN = zeros(n_eol, pat)
...
for w = 1:n_eol
    for t = 1:pat
        ...
        elseif oper_eol[w, 8] == "N"
            Psup_eolN[w, t] = Psup_eol[w, t]
        end
    end
end

Psup_ufvNE = zeros(n_ufv, pat)
Psup_ufvSE = zeros(n_ufv, pat)

for s = 1:n_ufv
    for t = 1:pat
        if oper_ufv[s, 8] == "SE"
            Psup_ufvSE[s, t] = Psup_ufv[s, t]
        elseif oper_ufv[s, 8] == "NE"
            Psup_ufvNE[s, t] = Psup_ufv[s, t]
        end
    end
end
...
R_upUHE = zeros(pat)
for t in 1:pat
    R_upUHE[t] = 0.04 * d[t] + 0.15 * sum(Psup_eolS[:, t]) + 0.06 * sum(Psup_eolNE[:, t])
end
R_dnUHE = zeros(pat)
for t in 1:pat
    R_dnUHE[t] = 0.025 * d[t] + 0.15 * sum(Psup_eolS[:, t]) + 0.06 * sum(Psup_eolNE[:, t])
end
...
#  Função Objetivo 
@objective(UCPDO, Min, sum(cvu[i]*pit[i,t] + cup[i]*(r_up[i,t] + r_dn[i,t]) for i=1:n_ugen for t=2:int) + α)
...
#-------------------------------------------------------------------------------
    ...
    # Reservas hidráulicas exogenas
    # gi_ReservesUP[t=1:pat], sum(r_up_h[h,t+1] for h=1:n_ugh) >= R_upUHE[t]
    # gi_ReservesDN[t=1:pat], sum(r_dn_h[h,t+1] for h=1:n_ugh) >= R_dnUHE[t]

    #Reservas hidráulicas endogenas 4 (beta it)
...
#-------------------------------------------------------------------------
# sol1 = 1.18394650235443e12 # kt  (90%)
# sol2 = 1.18395679260572e12 # kit (beta = 1)
# diff = abs(sol1 - sol2) ./ max.(sol1, sol2)
# if maximum(diff) < 1e-4  # Comparando com a tolerância do solver
#     println("As soluções estão dentro da tolerância.")
# else
#     println("Diferença detectada fora da tolerância.")
# end

#-------------------------------------------------------------------------
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados SIN - Cenário 4 =========")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_GerCSV_SIN.csv", PI, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCH_GerCSV_SIN.csv", GI, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_ResUP_SIN.csv", RUP, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_ResDN_SIN.csv", RDN, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCH_X_SIN.csv", X, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_Vol.csv", VOL, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_PcutEOL_SIN.csv", PcutEOL, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_PcutUFV_SIN.csv", PcutUFV, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_GerEOLSup_SIN.csv", Psup_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_GerUFVSup_SIN.csv", Psup_ufv, ',') #### exporta para arquivo .csv
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_GerEOLInf_SIN.csv", Pinf_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_CI_nsimu_SIN.csv", CI_nsimu, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-DemandaLiq_SIN.csv", d, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-DemandaBrut_SIN.csv", dbrut, ',') #### demanda bruta
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ploadrcbrut.csv", pload_rbrut, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_rdnh.csv", RDNH, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ruph.csv", RUPH, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_Z.csv", Z, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_FCF.csv", ALPHA, ',') #### exporta para arquivo .csv

results_ute = PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
X        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCH_X_SIN.csv",DataFrame,header=false)
PcutEOL  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ploadrcliq.csv",DataFrame,header=false)
RDNH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_rdnh.csv",DataFrame,header=false)
RUPH   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_ruph.csv",DataFrame,header=false)
Z      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4-Result_FCF.csv",DataFrame,header=false)
...
plot!(d,label=false,color=:red3,linewidth=3,linestyle=:dot,legend=:outerbottomright)
...
# Reservas hidráuliacs
reservesDN = vec(sum(RDNH;dims=1))[2:end]
reservesUP = vec(sum(RUPH;dims=1))[2:end]
ResUP_uhe_plot3 = uhe_plot + ute_plot + reservesUP
ResDN_uhe_plot3 = uhe_plot + ute_plot - reservesDN
...
plot(reservesUP,label="Upward",title="Caso 4 - Hydro reserves dispatch",linewidth=3)
plot!(reservesDN, label="Downward",linewidth=3)
xlabel!("Half-Hour")
ylabel!("MW")

#----------------------------------------------------------------------
# Cortes
plot(sum(PcutUFV,dims=1)',label=false,title="Total solar power curtailment",color=:gold2,linewidth=2,ylabel="MW", xlabel="Half-Hour")
plot(sum(PcutEOL,dims=1)',label=false,title="Total wind power curtailment",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

# Previsões
plot(sum(Psup_eol,dims=1)',label=false, title="Wind generation forecast for day-ahead scheduling",color=:green,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(Psup_ufv,dims=1)',label=false, title="Solar generation forecast for day-ahead scheduling",color=:gold2,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
...
plot(PcutEOL',title="Cortes EOL para dia seguinte",legend=:outerbottomright,linewidth=2,label=false)

plot(mean(PcutEOL./Psup_eol*100,dims=1)',title="Ratio of Wind Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:green)
xlabel!("Half-Hour")
ylabel!("%")

plot(mean(PcutUFV./Psup_ufv,dims=1)',title="Ratio of Solar Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:gold2)
xlabel!("Half-Hour")
ylabel!("%")

# Mapas de calor
heatmap(perc_eol_pat, title="Usinas EOL x Incerteza ao longo do dia")
heatmap(Matrix(perc_ufv_pat), title="Usinas UFV x Incerteza ao longo do dia")

include("./EOLUFV_previsao.jl")
perc_eol_cplx, PcutEOL_cplx, GerMax_eol_cplx, Psup_eol_cplx = Perc_Complex_Func(perc_eol_pat,pat, PcutEOL, GerMax_eol, Psup_eol)

heatmap1 = heatmap(perc_eol_cplx, title="Case 4 - Complex WPP \n uncertainty coefficients",xlabel="Half-hour",yflip=true)
plot!(size=[500,700])
ylabel!("WPP")

PcutEOL_GerMaxEOL = PcutEOL_cplx./GerMax_eol_cplx
norm_PcutEOL_GerMaxEOL = (PcutEOL_GerMaxEOL .- minimum(PcutEOL_GerMaxEOL)) ./ (maximum(PcutEOL_GerMaxEOL) - minimum(PcutEOL_GerMaxEOL))

heatmap2 = heatmap(PcutEOL_cplx./GerMax_eol_cplx, title="Case 4 - Ratio of Wind Curtail \n to Installed Capacity",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")
...
# --------------------------------   Modelo  Caso 4 - Dados Verificados ----------------------------------
...
@objective(UCPDO_ver, Min, sum(cvu[i]*results_ute[i,t] + cup[i]*(RUP[i,t] + RDN[i,t]) for i=1:n_ugen for t=2:int) + 
                       sum(rpo[h]*(RUPH[h,t] + RDNH[h,t]) for h=1:n_ugh for t=2:int) + 100000*sum(corte_d[t] for t=1:pat) +sum(ALPHA))
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho SIN - Caso 4 Dados Verificados ==============") 
println("Corte de carga: ", round(sum(Corte_carga);digits=2)/2," MWh, ", round(sum(Corte_carga)/sum(d_ver)*100;digits=5), " %")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso4/Caso4_Ver-Result_FCF2.csv",DataFrame,header=false)
...
#----------------------------------------------------------------
# Gráfico do despacho com dados verificados
#----------------------------------------------------------------

# Resultado final - SIN - Cenário 4
ute_plot = vec(sum(results_ute[:,2:end],dims=1))
uhe_plot = vec(sum(Ger_hidrTR,dims=1))[2:end]
uhe_plot2 = vec(sum(results_uhe,dims=1))[2:end]

g_eol2 = Ver_eol[:,:].-PcutEOL2
eol_plot = sum(Ver_eol[:,:],dims=1)'
eol_plot2 = sum(g_eol2,dims=1)'
eol_plot3 = sum(PcutEOL2,dims=1)'

g_ufv2 = Ver_ufv[:,:].-PcutUFV2
ufv_plot = sum(Ver_ufv[:,:],dims=1)'
ufv_plot2 = sum(g_ufv2,dims=1)'
ufv_plot3 = sum(PcutUFV2,dims=1)'

plot1 = plot(d_ver,label="Verified demand",color=:red3,linewidth=3,linestyle=:dot,legend=:outerbottomright)
areaplot!(eol_plot2+ufv_plot2+uhe_plot+ute_plot+Corte_carga, color=:red3,label="Unserved energy",fillalpha = 1,legend=:outerbottomright)
...
plot!(eol_plot + ufv_plot + ute_plot + uhe_plot + Corte_carga, fillrange = eol_plot2  + ufv_plot2 + ute_plot + uhe_plot + Corte_carga, fillstyle = :\,color=:gold2, label = "PPP curtail", legend=:outerbottomright,linewidth=2)
areaplot!(eol_plot + ufv_plot2 + ute_plot + uhe_plot + Corte_carga, color=:white,label=false,fillalpha = 1)
plot!(eol_plot + ufv_plot2 + ute_plot + uhe_plot, fillrange = eol_plot2 + ufv_plot2  + ute_plot + uhe_plot, fillstyle = :/,color=:green, label = "WPP curtail", legend=:outerbottomright,linewidth=2)

areaplot!(eol_plot2+ufv_plot2+uhe_plot+ute_plot+Corte_carga, color=:red3,label=false,fillalpha = 1)
areaplot!(eol_plot2+ufv_plot2+uhe_plot+ute_plot, color=:gold2,label=false,fillalpha = 1)
areaplot!(eol_plot2+uhe_plot+ute_plot, color=:green,label=false,fillalpha = 1)
areaplot!(uhe_plot+ute_plot,color=:dodgerblue3,label=false,fillalpha = 1)
areaplot!(ute_plot, color=:orange2,label=false,fillalpha = 1)

#plot!(eol_plot+ufv_plot2+ute_plot+uhe_plot+Corte_carga, fillrange = d_ver, fillstyle = :/,color=:green, label = "WPP curtail", legend=:outerbottomright)
#plot!(d_ver,label=false,color=:red3,linewidth=3,linestyle=:dot,legend=:outerbottomright)

xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 4")
plot!(size=[900,600])
plot!(d_ver,label=false,color=:red3,linewidth=2,linestyle=:dot,legend=:outerbottomright)
...
# Gráfico - geração hidráulica despachada em tempo real de forma dinâmica considerando as reservas up e down
plot(sum(results_uhe[:,2:end],dims=1)',label="HPP day-ahead dispatch",linewidth=2,legend=:topleft)
plot!(sum(results_uhe[:,2:end].+RUPH[:,2:end],dims=1)', label="Upward Reserves",linewidth=2)
plot!(sum(results_uhe[:,2:end].-RDNH[:,2:end],dims=1)', label="Downward Reserves",linewidth=2)
plot!(sum(Ger_hidrTR[:,2:end],dims=1)', label="Real HPP generation",linewidth=2)
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 4")

# Corte de carga
plot(Corte_carga,linewdth=2,color=:red3,title="Case 4 - Load Shedding",label=false)
xlabel!("Half-Hour")
ylabel!("MW")

# Cortes
plot(sum(PcutUFV2,dims=1)',label=false,title="Case 4 - Total solar power curtailed",color=:gold2,linewidth=2,ylabel="MW", xlabel="Half-Hour")
plot(sum(PcutEOL2,dims=1)',label=false,title="Case 4 - Total wind power curtailed",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
