...
println("====================================================================================================")
println("         Model Day-ahead Dispatch - Case 1     ")
println("====================================================================================================")
#-------------------------------------------------------------------------------
# Leitura de dados
#-------------------------------------------------------------------------------
...
deltaG_eol, deltaG_ufv = data_DelgaG_VRE(n_eol,n_ufv,pat)
...
#writedlm(scriptPath * "/Result_Psuph_SIN.csv", Psuph, ',')
...
pload_rbrut[:,1] .= 0
plot(sum(pload_rbrut,dims=1)'[2:end],label="ploadrbrut")
dbrut=sum(pload_rbrut,dims=1)[2:end]
plot!(dbrut,label="dbrut")
...
#-------------------------------------------------------------------------------
    ...
    fcf_cort[p=1:ncortes],   α >= sum(coef_ang[h,p]*vol[h,int]*1000 for h=1:n_uhe) + coef_ind[p]*1000
#-------------------------------------------------------------------------------
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados SIN - Cenário 1 =========")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_GerCSV_SIN.csv", PI, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_GerCSV_SIN.csv", GI, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_ResUP_SIN.csv", RUP, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_ResDN_SIN.csv", RDN, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_ResUP_SIN.csv", RUPH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_ResDN_SIN.csv", RDNH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_X_SIN.csv", X, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_Vol.csv", VOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_PcutEOL_SIN.csv", PcutEOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_PcutUFV_SIN.csv", PcutUFV, ',') 

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_GerEOLSup_SIN.csv", Psup_eol, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_GerEOLInf_SIN.csv", Pinf_eol, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_CI_nsimu_SIN.csv", CI_nsimu, ',') 

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-DemandaLiq_SIN.csv", d, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-DemandaBrut_SIN.csv", dbrut, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_ploadrcbrut.csv", pload_rbrut, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_ploadrcliq.csv", pload_r, ',') 

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_rdnCAG.csv", RDNH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_rupCAG.csv", RUPH, ',') 

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_rdnh.csv", RDNH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_ruph.csv", RUPH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_Z.csv", Z, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_FCF.csv", ALPHA, ',')

results_ute = PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
RUPH      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_ResUP_SIN.csv",DataFrame,header=false)
RDNH      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_ResDN_SIN.csv",DataFrame,header=false)
X         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_X_SIN.csv",DataFrame,header=false)
VOL       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_Vol.csv",DataFrame,header=false)
PcutEOL   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_ploadrcliq.csv",DataFrame,header=false)
RDNH    = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_rdnCAG.csv",DataFrame,header=false)
RUPH    = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_rupCAG.csv",DataFrame,header=false)
Z         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1-Result_FCF.csv",DataFrame,header=false)
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 2 - Model results with HUC and fixed reserves")
plot!(size=[960,600])
...
plot(reservesUP,label="Upward",title="Caso 1 - Fixed hydro reserves dispatch",linewidth=3)
...
# Mapas de calor
heatmap(perc_eol_pat2, title="Usinas EOL x Incerteza ao longo do dia")
heatmap(Matrix(perc_ufv_pat1), title="Usinas UFV x Incerteza ao longo do dia")
...
# --------------------------------   Modelo  Caso 1 - Dados Verificados ----------------------------------
...
@objective(UCPDO_ver, Min, sum(cvu[i]*results_ute[i,t] + cup[i]*(RUP[i,t] + RDN[i,t]) for i=1:n_ugen for t=2:int) + 
                       sum(rpo[h]*(RDNH[h,t] + RUPH[h,t]) for h=1:n_ugh for t=2:int) + 100000*sum(corte_d[t] for t=1:pat) +sum(ALPHA))
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho SIN - Caso 1 Dados Verificados ==============") 
println("Corte de carga: ", round(sum(Corte_carga);digits=2)/2," MWh, ", round(sum(Corte_carga)/sum(d_ver)*100;digits=2), " %")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1/Caso1_Ver-Result_FCF2.csv",DataFrame,header=false)
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 2 - Dispatch ajusted with \n verified load and wind generation data - Capalin 1.6x")
plot!(size=[900,600])
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Case 2 - Dispatch ajusted with \n verified load and wind generation data")
