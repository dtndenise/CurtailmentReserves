...
gr()
println("====================================================================================================")
println("         Model Day-ahead Dispatch - Caso 1b     ")
println("====================================================================================================")
#-------------------------------------------------------------------------------
# LeiLtura de dados
#-------------------------------------------------------------------------------
...
for w = 1:n_eol
    if oper_eol[w, 8] == "S"
        GerMax_Seol += GerMax_eol[w]
    elseif oper_eol[w, 8] == "NE"
        GerMax_NEeol += GerMax_eol[w]
    elseif oper_eol[w, 8] == "N"
        GerMax_Neol += GerMax_eol[w]
    end
end
...
for w = 1:n_eol
    for t = 1:pat
        ...
        elseif oper_eol[w, 8] == "N"
            kappa_eol_ResUP[w, t] = 0.0
            kappa_eol_ResDN[w, t] = 0.0
        end
    end
end
...
for s = 1:n_ufv
    for t = 1:pat
        if oper_ufv[s, 8] == "SE"
            kappa_ufv_ResUP[s, t] = 0.0
            kappa_ufv_ResDN[s, t] = 0.0
        elseif oper_ufv[s, 8] == "NE"
            kappa_ufv_ResUP[s, t] = 0.0
            kappa_ufv_ResDN[s, t] = 0.0
        end
...
kappa_eol_ResUP = round.(sparse(Matrix{Float32}(kappa_eol_ResUP)),digits=2)
kappa_eol_ResDN = round.(sparse(Matrix{Float32}(kappa_eol_ResDN)),digits=2)
kappa_ufv_ResUP = round.(sparse(Matrix{Float32}(kappa_ufv_ResUP)),digits=2)
kappa_ufv_ResDN = round.(sparse(Matrix{Float32}(kappa_ufv_ResDN)),digits=2)
...
# Resultados
#-------------------------------------------------------------------------
println("======== Resultados SIN - Cenário 1b (fixed reserves updated, solar and curtail) =========")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_GerCSV_SIN.csv", PI, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_GerCSV_SIN.csv", GI, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_ResUP_SIN.csv", RUP, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_ResDN_SIN.csv", RDN, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_ResUP_SIN.csv", RUPH, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_ResDN_SIN.csv", RDNH, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_X_SIN.csv", X, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_VOL_SIN.csv", VOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_PcutEOL_SIN.csv", PcutEOL, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_PcutUFV_SIN.csv", PcutUFV, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_GerEOLSup_SIN.csv", Psup_eol, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
#writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso4-Result_GerEOLInf_SIN.csv", Pinf_eol, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_CI_nsimu_SIN.csv", CI_nsimu, ',')

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_DemandaLiq_SIN.csv", d, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_DemandaBrut_SIN.csv", dbrut, ',') #### demanda bruta
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_ploadrcbrut.csv", pload_rbrut, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_ploadrcliq.csv", pload_r, ',')
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_Z.csv", Z, ',') 
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_FCF.csv", ALPHA, ',') 

results_ute=PI
#-------------------------------------------------------------------
# Leitura dos resultados

PI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
GI       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
RUP      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_ResUP_SIN.csv",DataFrame,header=false)
RDN      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCT_ResDN_SIN.csv",DataFrame,header=false)
RUPH     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_ResUP_SIN.csv",DataFrame,header=false)
RDNH     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_ResDN_SIN.csv",DataFrame,header=false)
X        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_X_SIN.csv",DataFrame,header=false)
PcutEOL  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_PcutUFV_SIN.csv",DataFrame,header=false)
Psup_eol  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_GerEOLSup_SIN.csv",DataFrame,header=false)
Psup_ufv  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_GerUFVSup_SIN.csv",DataFrame,header=false)
CI_nsimu  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
d         = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_DemandaLiq_SIN.csv",DataFrame,header=false)
dbrut     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_DemandaBrut_SIN.csv",DataFrame,header=false)
pload_rbrut = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_ploadrcbrut.csv",DataFrame,header=false)
pload_r  = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_ploadrcliq.csv",DataFrame,header=false)
Z      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_Z.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Result_FCF.csv",DataFrame,header=false)
...
#----------------------------------------------------------------------
# Reservas hidráulicas
plot(reservesUP,label="Upward",title="Caso 1b - Hydro reserves dispatch",linewidth=3)
...
#----------------------------------------------------------------------
# Cortes
plot(sum(PcutUFV,dims=1)',label=false,title="Caso 1b - Total solar power curtailment",color=:gold2,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(PcutEOL,dims=1)',label=false,title="Caso 1b - Total wind power curtailment",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

# Previsões
plot(sum(Psup_eol,dims=1)',label=false, title="Caso 1b - Wind generation forecast for day-ahead scheduling",color=:green,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(Psup_ufv,dims=1)',label=false, title="Caso 1b - Solar generation forecast for day-ahead scheduling",color=:gold2,linewidth=3) 
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
...
plot(PcutEOL',title="Caso 1b - Wind curtailment for day-ahead dispatch",legend=:outerbottomright,linewidth=2,label=false)

plot(mean(PcutEOL./Psup_eol*100,dims=1)',title="Caso 1b - Ratio of Wind Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:green)
xlabel!("Half-Hour")
ylabel!("%")

plot(mean(PcutUFV./Psup_ufv,dims=1)',title="Caso 1b - Ratio of Solar Energy Curtail to Forecasted Generation",linewidth=2,legend=false,color=:gold2)
xlabel!("Half-Hour")
ylabel!("%")


# Mapas de calor por COMPLEXO EOL (95)
include("./EOLUFV_resultadosR_testeTime.jl")
perc_eol_cplx, PcutEOL_cplx, GerMax_eol_cplx, Psup_eol_cplx = Perc_Complex_Func(perc_eol_pat2,pat, PcutEOL, GerMax_eol, Psup_eol)

heatmap1 = heatmap(perc_eol_cplx, title="Caso 1b - Complex WPP \n uncertainty coefficients",xlabel="Half-hour",yflip=true)
plot!(size=[500,700])
ylabel!("WPP")
...
heatmap2 = heatmap(PcutEOL_cplx./GerMax_eol_cplx, title="Caso 1b - Ratio of Wind Curtail \n to Installed Capacity",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap2b = heatmap(norm_PcutEOL_GerMaxEOL, title="Caso 1b - Ratio of Wind Curtail \n to Installed Capacity Norm",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap2c = heatmap(PcutEOL_cplx./Psup_eol_cplx, title="Caso 1b - Ratio of Wind Curtail \n to Forecasted Generation",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap3 = heatmap(PcutEOL_cplx, title="Caso 1b - Wind power\n plants' curtailment",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

plot(heatmap1, heatmap2c, heatmap3, layout = (1,3))
plot!(size=[1500,900])


# Mapas de calor UFV
heatmap(Matrix(perc_ufv_pat1), title="Caso 1b - Solar power plants' uncertainty coefficients",xlabel="Half-hour",yflip=true)
plot!(size=[500,700])
ylabel!("WPP")

heatmap(PcutUFV./GerMax_ufv, title="Caso 1b - Ratio of Solar Curtail to Installed Capacity",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")

heatmap(PcutUFV, title="Caso 1b - Solar power plants' curtailment",yflip=true)
plot!(size=[500,700])
xlabel!("Half-Hour")
ylabel!("WPP")
...
@objective(UCPDO_ver, Min, sum(cvu[i]*results_ute[i,t] + cup[i]*(RUP[i,t] + RDN[i,t]) for i=1:n_ugen for t=2:int) + 100000*sum(corte_d[t] for t=1:pat) +sum(ALPHA))
...
#-------------------------------------------------------------------------
println("========== Métricas de desempenho SIN - Caso 1b Dados Verificados ==============")
...
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
writedlm(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv

#-------------------------------------------------------------------
# Leitura dos resultados

results_ute       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_UCT_GerCSV_SIN.csv",DataFrame,header=false)
Ger_hidrTR        = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_UCH_GerCSV_SIN.csv",DataFrame,header=false)
PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
Ver_eol           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_VerEOL_SIN.csv",DataFrame,header=false)
Ver_ufv           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_VerUFV_SIN.csv",DataFrame,header=false)
d_ver             = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-DemandaVerLiq_SIN.csv",DataFrame,header=false)
Corte_carga       = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-CorteCarga_SIN.csv",DataFrame,header=false)
pload_r           = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_ploadrcliq.csv",DataFrame,header=false)
Corte_carga_bus   = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_CorteCargabus.csv",DataFrame,header=false)
Z2      = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_Z2.csv",DataFrame,header=false)
ALPHA     = CSV.read(scriptPath * "/RESULTADOS/RPOzero/Caso1b/Caso1b_Ver-Result_FCF2.csv",DataFrame,header=false)
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Caso 1b - Verified generation")
plot!(size=[900,600])
...
xlabel!("Half-Hour")
ylabel!("MW")
title!("SIN Caso 1b - Verified generation - Hydro reserves")

# Corte de carga
plot(Corte_carga,linewdth=2,color=:red3,title="Caso 1b - Load Shedding",label=false)
xlabel!("Half-Hour")
ylabel!("MW")

# Cortes
plot(sum(PcutUFV2,dims=1)',label=false,title="Total solar power curtailment",color=:gold2,linewidth=3,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(PcutEOL2,dims=1)',label=false,title="Total wind power curtailment",color=:green,linewidth=2,ylabel="MW", xlabel="Half-Hour")
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

# Geração EOL e UFV verificada
plot(sum(Psup_eol,dims=1)',label="Forecasted generation", title="Wind power generation",color=:green,linewidth=3) 
plot!(sum(Ver_eol,dims=1)',label="Verified generation",linewidth=3)
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])

plot(sum(Psup_ufv,dims=1)',label="Forecasted generation", title="Solar power generation",color=:gold2,linewidth=3) 
plot!(sum(Ver_ufv,dims=1)',label="Verified generation",linewidth=3)
xlabel!("Half-Hour")
ylabel!("MW")
plot!(size=[700,450])
