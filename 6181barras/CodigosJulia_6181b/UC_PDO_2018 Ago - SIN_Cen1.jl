...
#Psup_ufvh, Psup_eolh, perc_eol, perc_ufv, MW_eol, MW_ufv = data_PforecastVRE(n_eol,n_ufv,pat)
Psup_ufvh, Psup_eolh = data_PforecastVRE(n_eol,n_ufv,pat)

deltaG_eol = Matrix{Float32}(deltaG_eol)
deltaG_ufv = Matrix{Float32}(deltaG_ufv)
Psup_ufvh = Matrix{Float32}(Psup_ufvh)
Psup_eolh = Matrix{Float32}(Psup_eolh)

deltaG_eol = sparse(deltaG_eol)
deltaG_ufv = sparse(deltaG_ufv)
Psup_ufvh = sparse(Psup_ufvh)
Psup_eolh = sparse(Psup_eolh)

# GERAÇÃO EÓLICA - Passagem para semi-horario
deltaG_eol_pat = zeros(n_eol, pat)
for w in 1:n_eol
    for h in 1:Int(pat / 2)
        deltaG_eol_pat[w, 2*h-1] = deltaG_eol[w, h]
        deltaG_eol_pat[w, 2*h] = deltaG_eol_pat[w, 2*h-1]
    end
end
deltaG_eol_pat
...
deltaG_ufv_pat = zeros(n_ufv, pat)
for s in 1:n_ufv
    for h in 1:Int(pat / 2)
        deltaG_ufv_pat[s, 2*h-1] = deltaG_ufv[s, h]
        deltaG_ufv_pat[s, 2*h] = deltaG_ufv_pat[s, 2*h-1]
    end
end
deltaG_ufv_pat
...
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCT_GerCSV_SIN.csv", PI, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_GerCSV_SIN.csv", GI, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCT_ResUP_SIN.csv", RUP, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCT_ResDN_SIN.csv", RDN, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_ResUP_SIN.csv", RUPH, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_ResDN_SIN.csv", RDNH, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_X_SIN.csv", X, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_Vol.csv", VOL, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_PcutEOL_SIN.csv", PcutEOL, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_PcutUFV_SIN.csv", PcutUFV, ',') 

# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_GerEOLSup_SIN.csv", Psup_eol, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_GerUFVSup_SIN.csv", Psup_ufv, ',')
# #writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_GerEOLInf_SIN.csv", Pinf_eol, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_CI_nsimu_SIN.csv", CI_nsimu, ',') 

# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-DemandaLiq_SIN.csv", d, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-DemandaBrut_SIN.csv", dbrut, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_ploadrcbrut.csv", pload_rbrut, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_ploadrcliq.csv", pload_r, ',') 

# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_rdnh.csv", RDNH, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_ruph.csv", RUPH, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_Z.csv", Z, ',') 
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_FCF.csv", ALPHA, ',')
...
# VOL       = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_Vol.csv",DataFrame,header=false)
...
# RDNH    = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_ResUP_SIN.csv",DataFrame,header=false)
# RUPH    = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1-Result_UCH_ResDN_SIN.csv",DataFrame,header=false)
...
@objective(UCPDO_ver, Min, sum(cvu[i]*results_ute[i,t] + cup[i]*(RUP[i,t] + RDN[i,t]) for i=1:n_ugen for t=2:int) + 
                       ...
                       penalty_curt*sum(pcut_eol[w,t] for w=1:n_eol for t=1:pat) + 10000*sum(pcut_ufv[s,t] for s=1:n_ufv for t=1:pat) + sum(ALPHA))
...
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_UCT_GerCSV_SIN.csv", results_ute, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_UCH_GerCSV_SIN.csv", Ger_hidrTR, ',') #### exporta para arquivo .csv

# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_PcutEOL_SIN.csv", PcutEOL2, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_PcutUFV_SIN.csv", PcutUFV2, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_VerEOL_SIN.csv", Ver_eol, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_VerUFV_SIN.csv", Ver_ufv, ',') #### exporta para arquivo .csv

# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-DemandaVerLiq_SIN.csv", d_ver, ',') #### demanda líquida
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-CorteCarga_SIN.csv", Corte_carga, ',') #### corte de carga de d_ver
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_ploadrcliq.csv", pload_r, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_CorteCargabus.csv", Corte_carga_bus, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_Z2.csv", Z2, ',') #### exporta para arquivo .csv
# writedlm(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_FCF2.csv", ALPHA, ',') #### exporta para arquivo .csv
...
# PcutEOL2          = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_PcutEOL_SIN.csv",DataFrame,header=false)
# PcutUFV2          = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_PcutUFV_SIN.csv",DataFrame,header=false)
...
# Z2      = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_Z2.csv",DataFrame,header=false)
# ALPHA     = CSV.read(scriptPath * "/RESULTADOS/Caso1/Caso1_Ver-Result_FCF2.csv",DataFrame,header=false)
